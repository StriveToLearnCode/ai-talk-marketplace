...
AI Talk 是一个显式调用的研发需求澄清与只读诊断工具。普通需求只确认会改变实施结果的范围、行为和资源语义；“为什么”“前端还是后端”等请求进入故障定位，UI 异常会直接补充浏览器运行态证据。
...
诊断模式是唯一的只读检索例外：它从用户指定的文件和行号出发，依次检查控制层、数据层和渲染层。遇到不显示、位置异常、遮挡、点击无效或修改后仍未生效等 UI 异常时，AI Talk 直接使用已连接的浏览器检查渲染条件、DOM、尺寸、层级、资源请求和截图，不转交其他 Skill，也不修改代码。
