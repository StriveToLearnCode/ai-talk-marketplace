...
AI Talk 是一个 Codex 插件，用于实际调用适用的项目或公司 Skill 做只读能力发现，并把自然语言研发需求整理成只包含有来源事实、可直接交给 Codex 的任务话术。
...
推荐确定不等于兼容性已验证。最终话术只要求后续 Codex 读取真实代码，并验证用户明确需求实际涉及的接入内容，不固定罗列组件完整 API。

## 严格事实模式

AI Talk 默认不补全需求。只有以下信息可以进入最终话术：用户明确内容、与任务直接相关且带来源的项目事实、Skill 实际返回的检索事实，以及用户明确选择的能力。

组件支持标题、按钮、事件或数据字段，只能说明组件具备这些能力，不代表本需求需要它们。用户只说“开发一个普通弹窗”时，AI Talk 不会自行增加标题、内容、确认、关闭、props、技术栈、样式方案或验收标准。

非阻塞缺失项直接省略；只有答案不同会改变实现方向时才追问。用户明确要求补全或建议时，相关内容会单列为“建议（非需求）”，确认前不会进入最终任务话术。
...
选择候选不代表兼容，也不代表候选的全部能力都是业务要求。只有后续 Codex 实际读取代码后才能给出 `confirmed_reuse`、`partial_reuse`、`incompatible` 或 `reference_only`。
