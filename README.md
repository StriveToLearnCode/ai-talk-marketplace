...
明确且信息充分的请求直接生成话术，不读取项目或调用其他 Skill。只有组件选型、能力复用、接口契约或关键项目事实确实影响任务方向时，才按需执行只读发现。AI Talk 不执行最终任务、不修改业务代码，也不提供确认卡或伪交互按钮。
...
明确的代码修改、Bug 定位和简单 review 都可走快速路径。路径、Bug、截图、接口或开发等关键词本身不会触发发现。

## 按需发现

以下情况进入 `discovery`：组件选型或复用、用户明确要求检索现有能力、任务必须引用尚未提供的权威契约或项目事实、只读发现可以解决方向性歧义。
