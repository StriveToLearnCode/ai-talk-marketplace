...
保存原始输入、处理路径、起止时间、总耗时、明确文件列表、实际读取文件与字节数、未读取文件及原因、提取的文件事实、Skill-only 命令和耗时、frontmatter 候选、话术中指定 Skill 及顺序、依赖读取数、下游 Skill/reference 调用数、外部工具调用数、最终提示词和固定未执行声明。所有执行类字段必须为 0。
