...
索引只解析 `.agents/skills/**/SKILL.md` frontmatter 的 `name` 和 `description`。Skill 正文、reference、脚本、组件、模板和历史实现不进入 AI Talk 上下文。业务文件只有在用户明确附带或给出精确路径时，才按最多 3 个、每个 64KB 的边界读取。
...
- 已明确提供的本地目标文件可以作为提示词上下文读取，但不得跟踪 imports、读取同目录文件或扫描项目。

## 禁止降级

Skill-only 无结果或失败时，不得运行 `collect_context.py`、完整索引、公司组件 Skill、项目搜索、浏览器或网络工具。保留真实失败原因，并在提示词中要求后续 Codex 自行检查适用 Skill。

AI Talk 阶段的固定目标是：项目命令最多 1 次；明确目标文件读取最多 3 次且每个最多 64KB；下游 Skill 调用、reference 读取、依赖读取、Figma、飞书、浏览器、代码修改和测试执行均为 0。
