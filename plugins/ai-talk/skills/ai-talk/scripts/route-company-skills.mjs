...
import { buildExecutionProtocol, formatUserOutput } from "./format-user-output.mjs";

const SKILL_ROOT = path.join(".agents", "skills");
const MAX_BYTES = 128 * 1024;
const MAX_PROJECT_FILES = 8;
const LOCAL_IMPORT_EXTENSIONS = ["", ".vue", ".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".json", ".css", ".scss", ".less"];
...
function contextValue(item) {
  return String(item?.value || "").replace(/^目标(?:文件|目录)：/, "").trim();
}
...
function hasForbiddenProjectSegment(candidate, root) {
  return projectRelative(root, candidate).split(path.sep).includes("node_modules");
}

async function safeProjectPath(root, candidate) {
  try {
    const resolved = await realpath(candidate);
    if (!within(resolved, root) || hasForbiddenProjectSegment(resolved, root)) return null;
    return resolved;
  } catch {
    return null;
  }
...
function localImportSpecifiers(content) {
  const specifiers = [];
  const add = (value) => {
    if (/^\.{1,2}\//.test(value) && !specifiers.includes(value)) specifiers.push(value);
  };
  for (const match of content.matchAll(/(?:from\s*|import\s*\(|require\s*\()\s*["']([^"']+)["']/g)) add(match[1]);
  for (const match of content.matchAll(/\bimport\s*["']([^"']+)["']/g)) add(match[1]);
  for (const match of content.matchAll(/@import\s+(?:url\()?\s*["']([^"']+)["']/g)) add(match[1]);
  return specifiers.slice(0, MAX_PROJECT_FILES);
}

async function resolveLocalImport(root, importer, specifier) {
  const base = path.resolve(path.dirname(importer), specifier);
  const candidates = LOCAL_IMPORT_EXTENSIONS.flatMap((extension) => [
    `${base}${extension}`,
    path.join(base, `index${extension}`),
  ]);
  for (const candidate of candidates) {
    const resolved = await safeProjectPath(root, candidate);
...
async function collectProjectContext(rootInput, confirmedContext, evidenceItems) {
  let root;
  try {
    root = await realpath(path.resolve(rootInput));
  } catch {
    return { items: [], unresolved: ["项目根目录不可读或无法确认边界。"] };
  }
  ...
  const targets = [];
  const add = (type, value, source) => {
    if (!value || items.some((item) => item.type === type && item.value === value)) return;
    items.push({ type, value, source });
  };
  const issue = (value) => {
    if (value && !unresolved.includes(value)) unresolved.push(value);
  };
  const readProjectFile = async (candidate, label) => {
    if (readFiles.has(candidate)) return readFiles.get(candidate);
    if (readFiles.size >= MAX_PROJECT_FILES) {
      issue(`项目上下文文件超过 ${MAX_PROJECT_FILES} 个，未继续读取 ${label}。`);
      return null;
    }
    try {
      const content = await readBounded(candidate);
      if (content === null) {
        issue(`${label} 超过 128 KiB，未读取内容。`);
        return null;
      }
      readFiles.set(candidate, content);
      return content;
    } catch {
      issue(`${label} 不可读。`);
      return null;
  ...
  for (const context of confirmedContext.filter((item) => ["target_file", "target_directory"].includes(item.type))) {
    const raw = contextValue(context);
    if (!path.isAbsolute(raw)) add(context.type, raw.replace(/^\.\//, ""), context.source);
    const candidate = path.isAbsolute(raw) ? path.resolve(raw) : path.resolve(root, raw);
    const resolved = await safeProjectPath(root, candidate);
    if (!resolved) {
      issue(`显式目标 ${raw} 不存在、不可读或超出项目根目录。`);
      continue;
    }
    let info;
    try {
    ...
    if (context.type === "target_file" && !info.isFile()) {
      issue(`显式目标 ${raw} 不是文件。`);
      continue;
    }
    if (context.type === "target_directory" && !info.isDirectory()) {
      issue(`显式目标 ${raw} 不是目录。`);
      continue;
    }
    const relative = projectRelative(root, resolved);
    add(context.type, relative, context.source);
    targets.push({ type: context.type, resolved, relative });
  }

  const agentFiles = [];
  for (const target of targets) {
    let current = target.type === "target_directory" ? target.resolved : path.dirname(target.resolved);
    const chain = [];
    ...
    for (const candidate of chain.reverse()) {
      const resolved = await safeProjectPath(root, candidate);
      if (!resolved || agentFiles.includes(resolved)) continue;
      const relative = projectRelative(root, resolved);
      if (await readProjectFile(resolved, relative)) {
        agentFiles.push(resolved);
        add("project_rule", relative, "project:AGENTS.md");
      }
    }
  }

  for (const target of targets.filter((item) => item.type === "target_file")) {
    const content = await readProjectFile(target.resolved, target.relative);
    if (content === null) continue;
    for (const specifier of localImportSpecifiers(content)) {
      if (readFiles.size >= MAX_PROJECT_FILES) break;
      const dependency = await resolveLocalImport(root, target.resolved, specifier);
      if (!dependency) continue;
      const relative = projectRelative(root, dependency);
      if (await readProjectFile(dependency, relative) !== null) add("direct_dependency", relative, `project:import:${target.relative}`);
    }
  }

  for (const item of evidenceItems) {
    const type = item.type === "api_document" ? "api_document"
      : item.type === "selected_code" ? "selected_code"
        : ["screenshot", "visual_design", "interaction_flow"].includes(item.type) ? item.type : null;
    if (type) add(type, item.detail, item.source);
  }
  return { items, unresolved };
}

function appendSemanticProjectContext(items, entities) {
  const result = [...items];
  const add = (type, value, source) => {
    if (!value || result.some((item) => item.type === type && item.value === value)) return;
    result.push({ type, value, source });
  };
  for (const item of entities.asset_resource || []) add("asset_resource", item.value, item.source);
  for (const item of entities.api || []) add("api", item.value, item.source);
  return result.slice(0, 16);
}

function defaultRulesFor(intent, query, projectContext) {
  const rules = [];
  const add = (value, source, evidence) => {
    if (!value || rules.some((item) => item.value === value)) return;
    rules.push({ value, source, ...(evidence ? { evidence } : {}) });
  };
  const mutating = ["feature_create", "feature_modify", "bug_fix", "automated_test"].includes(intent);
  add("不引入用户未确认的业务逻辑", "universal");
  if (mutating && !/(?:允许|可以|需要).{0,8}(?:跨模块|全局|无关模块)|(?:整体|全局)重构/.test(query)) {
  ...
  if (intentRule && !(/不(?:要|用|再)?复用|重新实现|重写/.test(query) && intentRule.includes("复用"))) add(intentRule, "intent");
  for (const context of projectContext.filter((item) => item.type === "project_rule")) {
    add(`遵循 ${context.value} 中适用于目标范围的项目规则`, "project", context.value);
  }
  const dependencies = projectContext.filter((item) => item.type === "direct_dependency");
  if (dependencies.length && !/不(?:要|用|再)?复用|重新实现|重写/.test(query)) {
    add("沿用目标文件及其直接依赖中的现有实现模式", "project", dependencies.map((item) => item.value).join(", "));
  }
...
export async function routeCompanySkills(args) {
  ...
  const collectedProjectContext = await collectProjectContext(root, confirmedContext, evidenceItems);
  const projectContext = appendSemanticProjectContext(collectedProjectContext.items, entities);
  const defaultRules = defaultRulesFor(intent, args.query, projectContext);
  const retrievalQueryGroups = queryGroupsFor(intent, entities);
  const unknowns = [...new Set([
    ...unknownsFor(profile, args.query, confirmedContext),
    ...collectedProjectContext.unresolved,
  ])];
  ...
  result.execution_goal = protocol.executionGoal || protocol.goal;
  result.skill_handoff = protocol.skillHandoff;
  return result;
}
