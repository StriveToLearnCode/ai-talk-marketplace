...
→ 将紧凑任务和 Skill 执行顺序写入一个自包含的 text 代码块
→ 立即停止
```
...
AI Talk 输出最多 4 行的“需求理解”摘要、一个自包含且可直接复制的 `text` 提示词代码块，以及：
...
普通任务的提示词默认不超过 20 行、约 900 个中文字符；复杂任务最多 30 行、约 1400 个中文字符。同一事实只出现一次，不重复输出证据和通用工程约束。
