...
AI Talk 用于开发前的需求澄清和只读诊断。它保留用户原话：普通需求只处理会改变实施结果的问题；“为什么”“前端还是后端”等请求沿控制、数据、渲染三层定位；UI 异常直接补充浏览器运行态证据。
...
6. UI 异常按页面状态、条件、DOM、布局层级、资源和截图顺序取证；浏览器不可用时明确降级，不伪造验证结果。
...
诊断请求固定使用 `inspect_only`，不推荐 `gen-code` 或 `ui-self-check`。Handoff 保存诊断事实与定责条件；用户说“执行”时直接续接上一轮证据，不重新扫描或升级为修改模式。浏览器只允许可逆的页面状态切换，不触发领取、提交、支付或写接口。
