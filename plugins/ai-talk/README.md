...
AI Talk 将自然语言研发需求整理成只包含有来源事实、可直接交给 Codex 的任务话术。它优先实际调用当前会话中适用的项目或公司 Skill 执行只读能力发现，再使用项目本地索引补充直接相关的项目事实和复用候选。
...
- `task.information_policy`：严格事实模式、允许来源和 Assumption 处理规则。
...
- 用户明确内容属于用户事实；项目文件和 Skill 结果必须保留自己的来源类型。
- 组件支持某项能力不代表用户要求该能力，不得把组件完整 API 写入任务。
- 项目技术栈只有在真实读取、直接相关时才作为“已验证约束”单列，不能伪装成用户需求。
- 非阻塞缺失项省略；阻塞项才追问。空字段不为填满模板生成内容。
- 用户明确要求补全或建议时，只能输出“建议（非需求）”，确认前不得进入最终话术。
...
- 不把候选发现写成兼容性结论或业务需求；后续 Codex 只检查用户明确需求实际涉及的接入内容。
