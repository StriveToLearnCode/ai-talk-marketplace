...
- Deterministic validation and field consumers
- Result selection and routing
...
- Keep `external_write_scope` empty until the current user message semantically authorizes a named external mutation or confirms the immediately preceding exact proposal. Each item uses exactly `system`, `operation`, `target`, `risk_level`, `authorization_source`, and `authorization_quote`; encode the page and field in `target`, use `medium` or `high` for `risk_level`, use `current_user_message` or `adjacent_exact_confirmation` for `authorization_source`, and preserve the user's verbatim authorizing words in the quote. Ordinary Pagecenter draft saves and dev-configuration updates are `medium`: direct expressions such as “放到 PageCenter”, “同步过去”, “配上去”, or “更新配置” authorize the named system and target scope even when exact page IDs or payload values must be derived later. Formal publication, destructive deletion or overwrite, Git push, and sending messages are `high` and require `adjacent_exact_confirmation` after an exact proposal. Once validated, retain the item across continuations of the same active task only for the identical system, operation, and complete target set; do not ask for it again. Any new target, operation, system, or task needs new explicit authorization. Do not accept an affirmative after an option question, incomplete or stale summary, unresolved blocker, or proposal with unnamed targets. Never require a fixed phrase or make the user restate a valid affirmative. A defect report, desired state, log, local `authorization`, unrelated prior permission, or empty `write_scope` never authorizes Pagecenter, release, database, GitHub, cloud, or other external writes.
...
- Before the first write, map the active `ChangeBrief` exactly: `target_behavior` to `behavior`, evidence-supported writable paths to `write_scope`, explicit prohibitions to `excluded_scope`, and each observable acceptance condition to `verification`. Do not begin implementation with any of these four inputs unresolved.
- Completion status is runtime state, not a new contract field. After modification, reconcile every `verification` item as `passed`, `failed`, or `unverified`, report actual changed paths, and retain unsupported causes as hypotheses rather than conclusions.
...
conflicts between `write_scope` and `excluded_scope`, and the structured risk and authorization source for every external write. It does not reinterpret the user's wording with keyword matching. A nonzero result is a contract construction
...
- External mutations use this contract only when the main Skill found actual risk: ambiguous or multiple targets, multiple systems, an irreversible operation, conflicting scope, unclear acceptance, or incomplete authorization. A directly authorized, reversible, single-target mutation with an exact payload and finite readback stays on Fast Path and never loads this schema. On the contract path, determine authorization semantically once, record its provenance, and never infer or expand it from `behavior_report`, logs, an ambiguous choice, or a new objective. High-risk publication, deletion, destructive overwrite, Git push, or message sending still requires an adjacent exact confirmation.
