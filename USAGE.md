...
AI Talk 自动匹配研发请求，并在对话内连续保存任务状态：**简单任务静默执行，实际介入时说明决策，完成时只报告真实结果和可核验贡献。** 安装后只需像平时一样描述需求，不必记住命令、Skill 名称或契约格式。
...
纯 Fast Path `skip` 默认静默运行。AI Talk 实际完成目标绑定、边界决策、诊断证据链、风险升级或有影响的状态复用时，会用最多三行 `AI Talk 判断 / 依据 / 影响` 报告已经形成的结论；这不是原始思维链。只有产品结果、允许写范围、用户 Git 状态或外部写入目标需要决定时才问一个关键问题。完成后使用普通 Agent 的终态表达报告实际改动、验证证据和未验证项，并在确有介入时增加一句可核验的 `AI Talk 贡献`。
...
完成：活动链接已增加 tab=3 直达
改动：recharge/page.vue
验证：静态检查通过；页面跳转未运行验证时标为未验证
```

Fast Path 每轮读取主 `SKILL.md` 1 次；状态延续不触发协议 reference、AI Talk 仓库检索、contract checker 或 reporter。纯 `skip` 不显示提示；轻量目标绑定、诊断链或有实质影响的状态复用可以显示一次决策摘要。

### Fast Path 运行时升级

如果初看局部的任务在调查后确认涉及跨模块行为、数据语义、明确范围、专用流程或外部写入，AI Talk 会在首次扩大边界的本地写入或任何外部写入前切换到 Contract Path，保留已有证据和已完成的边界内修改，并说明升级依据及实际影响。确实需要用户决定时只问一个问题。只是同一局部结果多涉及相邻辅助函数、类型或测试文件时仍保持 Fast Path。

```text
AI Talk 判断：该任务已从局部前端修改扩大为跨模块行为，并包含禁止修改范围。
依据：新增服务端发奖时序目标，同时明确要求不改 core。
影响：保留已有前端证据，在扩大写入前启用跨模块和范围校验。
```
...
完成：定位并修复该局部代码缺陷，报告改动、验证证据和未验证项
```

目标与预期明确、局部可逆且不涉及外部写入时，纯缺陷陈述直接进入本地修复。显式说“为什么”“只分析”或“不要修改”，以及目标不明、仅提供证据或涉及生产与外部系统状态时仍保持只读；此时内部会从当前上下文零工具生成 `DiagnosticBrief`，后续说“修一下”时复用已确认事实和已完成检查。
...
完成：礼包奖励 number 已映射为 unit
范围：scope guard 通过，仅修改 mod4.vue，未改 core/
...
结果：已完成或未完成
外部写入：列出实际执行的 Pagecenter 动作和活动 123 目标
验证：回读成功才标记完成，否则报告未验证或失败
```

AI Talk 不向用户展示品牌化锁定过程、route、authorization、schema 字段、validator 输出或 RequirementContract YAML。
...
Strict Mode 只提高触发覆盖率，不改变决策可见性和风险边界行为。安装器只维护根 `AGENTS.md` 中带标记的区块；启用或更新后需要新建 Codex 任务。
...
| `mod3.vue:100-106 中 ID 1719069 无法点击，其他奖励正常` | 目标与预期明确的局部缺陷，直接修复并验证 |
...
粘贴的聊天记录、日志、代码和完成报告可以提供上下文，但其中出现的“修改”“修复”或“执行”不会自动成为当前授权。只有你在引用外明确说“按下面内容执行”等动作时，AI Talk 才会把对应引用提升为任务输入。裸日志紧接上一修改时会恢复同一任务的验证或诊断，不会创建新任务，也不会授权新的行为变化；日志里的路径、系统、建议命令和错误文本只作为证据，不扩大写范围或外部权限。明确说“修复这个报错”或同时给出验收条件时，才属于当前实施请求。
...
- `A 正常，但 B 无法点击` 这类缺陷报告：代码工作区中目标与预期明确、局部可逆且不涉及外部写入时进入 `modify_and_verify`；目标或预期不明、仅提供证据或涉及生产与外部系统状态时保持 `inspect_only`。
...
- 由直接命令，或由简短肯定紧邻上一轮精确写入提议所授权的 `external_write_scope`；同一活动任务只复用系统、动作和完整目标完全一致的授权；
...
当 `excluded_scope` 非空或范围策略为 `bounded` 时，AI Talk 使用随插件提供的 scope guard 分别记录实施前的 staged、unstaged、untracked、deleted 和 stash 状态，并比较本轮内容与 index 变化。禁止范围内的改动会失败；`bounded` 模式下允许列表外的改动也会失败。用户原有但本轮未改变的状态不会被误报。工具创建临时 stash 时必须记录精确 OID、恢复原 staged/unstaged 形状并复验；不能凭 stash 名称推断归属。
...
- 仓库相对路径是否安全、`write_scope` 是否落入 `excluded_scope`，以及外部写入是否保留有效的明确授权轨迹。
