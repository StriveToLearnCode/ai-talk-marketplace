...
| Skill | 用途 | 是否自动执行 |
| --- | --- | --- |
| `$ai-talk:ai-talk` | 理解需求、建立证据对应关系并生成可复制提示词 | 否，必须显式调用 |
...
当前仓库同时包含开发副本和公开 marketplace。其他用户安装时，推荐 clone 仓库并注册公开 marketplace 子目录：
...
输出中应存在已启用的 `ai-talk@ai-talk-marketplace`。安装后请新建一个 Codex 对话，已经打开的旧对话不会自动加载新 Skill。
...
```text
$ai-talk:ai-talk 修改 src/page.vue 中的错误语法，保留现有业务逻辑

$ai-talk:ai-talk 结合我附带的页面截图，把图片区域与需求逐项对应，并生成后续 Codex 可直接执行的提示词

$ai-talk:ai-talk 读取本地 openapi.yaml，整理接口联调任务；不要打开外部链接

$ai-talk:ai-talk 审查 src/utils/reward.ts，只生成 review 提示词，不执行审查
```

AI Talk 会输出：

1. 首屏“目标”“事实”“自动补充”三组短摘要。
2. 默认关闭的“查看完整 Prompt”折叠区，内部是唯一的 fenced `text` 代码块。
3. 固定的未执行声明。

“事实”最多 3 条，只显示直接证据；“自动补充”最多 3 条，统一使用“执行时……”表述，不代表 AI Talk 当前轮已经读取项目资料。展开并复制完整 `text` 代码块，交给后续 Codex 执行即可；首屏摘要不是执行提示词的必要依赖。

普通任务的完整 Prompt 默认不超过 12 行、约 500 个中文字符；复杂任务最多 18 行、约 800 个中文字符。相同事实和路径不会重复出现，也不会输出固定的执行、交付或通用约束段落。
...
```text
1. 使用 $ai-talk:ai-talk 整理需求。
2. 检查首屏目标和事实是否符合原意。
3. 展开“查看完整 Prompt”，复制 text 代码块交给后续 Codex 执行。
...
确认 `ai-talk@ai-talk-marketplace` 已安装且启用，然后新建对话。不要在安装前已经打开的旧对话中验证。

### `$ai-talk` 和 `$ai-talk:ai-talk` 有什么区别

公开 marketplace 安装使用 `$ai-talk:ai-talk`。仓库维护者通过本地 `personal` marketplace 调试时使用 `$ai-talk`。普通安装者应使用公开调用名。
