...
const EXTERNAL_RISK_LEVELS = new Set(["medium", "high"]);
const EXTERNAL_AUTHORIZATION_SOURCES = new Set([
  "current_user_message",
  "adjacent_exact_confirmation",
]);
...
function verifyExternalWriteScope(contract, errors) {
  ...
  contract.external_write_scope.forEach((item, index) => {
    ...
    if (!EXTERNAL_RISK_LEVELS.has(item.risk_level)) {
      addFinding(errors, "external.invalid_risk_level", `${field}.risk_level`, `Unknown external risk level: ${item.risk_level}`);
    }
    if (!EXTERNAL_AUTHORIZATION_SOURCES.has(item.authorization_source)) {
      addFinding(errors, "external.invalid_authorization_source", `${field}.authorization_source`, `Unknown external authorization source: ${item.authorization_source}`);
    }
    if (item.risk_level === "high" && item.authorization_source !== "adjacent_exact_confirmation") {
      addFinding(errors, "external.high_risk_requires_confirmation", `${field}.authorization_source`, "High-risk external writes require an adjacent exact confirmation.");
    }
  });
  return true;
}
