...
const SKILLS_PARENT = path.dirname(SKILL_ROOT);
const INSTALLED_SKILLS_ROOT = path.basename(SKILLS_PARENT) === "skills"
  ? SKILLS_PARENT
  : path.join(SKILL_ROOT, "skills");
const COMPARISON_ROOT = path.join(SKILL_ROOT, "docs", "skills");
...
export async function routeCompanySkills(args) {
  ...
  const discovery = await discoverSkills({
    root: args.root,
    pluginSkillsRoot: INSTALLED_SKILLS_ROOT,
