---
name: ai-talk
description: 在用户显式调用 $ai-talk:ai-talk 并希望为研发任务选择公司 Skill 时，保留内部结构化检索画像，基于真实运行时 SKILL.md 推荐一个 Skill，并用约 150 字自然语言说明任务理解、执行建议、判断依据和一个必要的排除项。只做路由，不执行任务、不调用下游 Skill、不生成执行 Prompt。
---
