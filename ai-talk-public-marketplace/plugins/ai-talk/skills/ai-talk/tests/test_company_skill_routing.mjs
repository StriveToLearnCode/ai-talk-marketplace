...
import {
  ...
  formatUserOutput,
} from "../scripts/format-user-output.mjs";
...
const SCRIPT = path.resolve(
  import.meta.dirname,
  "../scripts/route-company-skills.mjs",
);
const CASES = path.resolve(
  import.meta.dirname,
  "company-skill-routing-cases.json",
);
...
const SKILLS = [
  [
    "fe-ai-test",
    "ai-test",
    "生成 Midscene 测试文件 midscene-test.ts、自动化测试用例和报告。",
  ],
  [
    "fe-gen-frontend-plan",
    "gen-frontend-plan",
    "从原型、OpenAPI、PageCenter 和 Figma UI Meta 生成 docs/plan 前端方案。",
  ],
  ["fe-gen-code", "gen-code", "生成活动前端页面、组件和业务逻辑代码。"],
  [
    "fe-figma-analyze",
    "figma-analyze",
    "分析 Figma 原型并输出交互 Markdown 方案。",
  ],
  [
    "figma-analyze",
    "figma-analyze",
  ...
  [
    "fe-figma-to-ui-meta",
    "figma-to-ui-meta",
    "调用 Mercury 将 Figma 转成 figma-ui-meta.json。",
  ],
  [
    "fe-gen-page-center-config",
    "gen-page-center-config",
    "生成 page-center-config.json 并推送配置。",
  ],
  [
    "fe-custom-components-skill",
    "custom-components-skill",
    "生成根据 uiMeta 渲染的活动积木组件。",
  ],
  [
    "fe-ui2-upgrade-guide",
    "ui2-upgrade-guide",
    "将 ui-components 升级到 ui2-components。",
  ],
...
async function fixture() {
  const root = await mkdtemp(
    path.join(os.tmpdir(), "ai-talk-company-fixture-"),
  );
...
async function routeDebug(root, prompt, evidenceTypes = [], ...extra) {
  ...
  args.push(...extra);
  return JSON.parse(
    (await execFileAsync(process.execPath, args, { encoding: "utf8" })).stdout,
  );
}
...
async function output(prompt, evidence = [], extra = []) {
  ...
  return (
    await run(process.execPath, args, { encoding: "utf8" })
  ).stdout.trim();
}
...
function assertCompactProtocol(output, { minChineseCharacters = 0 } = {}) {
  ...
  assert.ok(
    ...
    `protocol too short (${chineseCharacters} Chinese characters):\n${output}`,
  );
  assert.ok(
    lines.length <= 40,
    `protocol exceeds one screen (${lines.length} lines):\n${output}`,
  );
  const chinese = lines[0] === "任务目标：";
  assert.equal(lines[0], chinese ? "任务目标：" : "Goal", output);
  assert.ok(
    chineseCharacterCount(lines[1] || "") <= 50,
    `goal exceeds 50 Chinese characters:\n${output}`,
  );
  for (const old of [
    "研发语义",
    "Semantic Context",
  ...
  if (chinese) {
    assert.ok(
      lines.every(
        (line, index) =>
          index < 2 || chineseHeadings.has(line) || line.startsWith("- "),
      ),
      output,
    );
    for (const leaked of [
...
function assertCategorizedQueries(payload) {
  ...
  assert.deepEqual(Object.keys(payload.retrieval_query_groups), [
    "docs",
    "skills",
    "components",
    "code",
  ...
  for (const queries of Object.values(payload.retrieval_query_groups)) {
    assert.ok(
      queries.length <= 3,
      JSON.stringify(payload.retrieval_query_groups),
    );
    assert.equal(new Set(queries).size, queries.length);
    assert.ok(
      queries.every((query) => typeof query === "string" && query.trim()),
      JSON.stringify(queries),
    );
  }
  assert.deepEqual(
    payload.retrieval_queries,
    Object.values(payload.retrieval_query_groups).flat(),
  );
}
...
test("real repository index and company fixture index are reported separately", async (t) => {
  ...
  assert.deepEqual(real.routing.index.stats.by_scope, {
    companion: { files: 1, unique_names: 1 },
  });
...
test("real index reports duplicate names and excludes comparison copies", async (t) => {
  ...
  const payload = await routeDebug(
    root,
    "分析 Figma 原型并输出 Markdown 文档",
    [],
    "--source-root",
    `comparison=${COMPARE}`,
  );
  ...
  assert.deepEqual(
    payload.routing.index.duplicate_name_conflicts.map((item) => item.name),
    ["figma-analyze"],
  );
  assert.ok(
    payload.routing.index.warnings.some((warning) =>
      warning.includes("Excluded non-runtime"),
    ),
  );
  for (const item of [
    payload.routing.recommendation,
    ...payload.routing.alternatives,
  ].filter(Boolean)) {
...
test("indexes explicit applicability sections but ignores ordinary body", async (t) => {
  ...
  assert.ok(
    ![payload.routing.recommendation, ...payload.routing.alternatives]
      .filter(Boolean)
      .some((item) => item.name === "ignored-router"),
  );
});
...
test("benchmark prints confusion matrix and meets thresholds", async (t) => {
  ...
  const top1 =
    clear.filter((item) => item.predicted === item.expected).length /
    clear.length;
  const top3 =
    ambiguous.filter((item) => item.top3.includes(item.expected)).length /
    ambiguous.length;
  const errors = results.filter((item) =>
    item.clarity === "clear"
      ? item.predicted !== item.expected
      : !item.top3.includes(item.expected),
  );
    ...
    `${JSON.stringify(
      {
        benchmark: {
          ...
          clear_top1_accuracy: top1,
          ambiguous_top3_recall: top3,
        },
...
test("explicit live UI inspection wins over generic problem words", async (t) => {
  ...
  for (const prompt of prompts) {
    ...
    assert.equal(
      payload.routing.retrieval_profile.desired_output,
      "live_ui_findings",
      prompt,
    );
  }
...
test("screenshot evidence requires an attachment flag or an explicit reference phrase", async (t) => {
  ...
  for (const prompt of [
    ...
    const payload = await routeDebug(root, prompt);
    assert.ok(
      !payload.routing.retrieval_profile.evidence_types.includes("screenshot"),
      prompt,
    );
    assert.ok(
      !payload.confirmed_context.some((item) =>
        item.type.includes("screenshot"),
      ),
      prompt,
    );
  }
  ...
  assert.ok(
    attachment.routing.retrieval_profile.evidence_types.includes("screenshot"),
  );
});
...
test("existing UI bugs default to code fixes unless explicitly analysis-only", async (t) => {
  ...
  const analysis = await routeDebug(
    root,
    "这个已有页面显示异常，只分析原因，不修改代码",
  );
  assert.equal(analysis.execution_skill, "ui-self-check");
  assert.equal(
    analysis.routing.retrieval_profile.desired_output,
    "live_ui_findings",
  );
});
...
test("multiple image attachments keep visual, interaction, and API roles with sources", async (t) => {
  ...
  assert.deepEqual(
    payload.confirmed_context.slice(0, 3).map((item) => item.type),
    ["visual_design", "interaction_flow", "api_document"],
  );
  assert.deepEqual(
    payload.confirmed_context.slice(0, 3).map((item) => item.source),
    ["attachment:1", "attachment:2", "attachment:3"],
  );
  assert.equal(payload.intent, "feature_create");
  assert.ok(values(payload, "ui_component").includes("dialog"));
  assert.ok(
    values(payload, "target_scope").includes("recharge/components/dialogs"),
  );
  assert.deepEqual(payload.retrieval_query_groups.components, [
    "dialog",
    "modal",
    "popup",
  ]);
...
test("an explicit file bug stays compact and captures only the target and symptom", async (t) => {
  ...
  assert.ok(
    payload.retrieval_query_groups.code.some((query) =>
      query.includes("src/components/reward-card.vue"),
    ),
  );
...
test("a generic dialog request expands retrieval vocabulary without inventing requirements", async (t) => {
  ...
  for (const invented of ["确认按钮", "奖励列表", "props", "事件"]) {
    assert.ok(
      !JSON.stringify(payload).includes(invented),
...
test("image words without attachments never become screenshot context", async (t) => {
  ...
  assert.ok(
    !payload.confirmed_context.some((item) => item.type.includes("screenshot")),
  );
...
test("coding tasks keep gen-code internal while output centers context and retrieval", async (t) => {
  ...
  for (const prompt of [
    ...
    assert.ok(
      !output.includes(JSON.stringify(payload.retrieval_queries)),
      output,
    );
...
test("RTL reward progress screenshot produces source-backed concepts and categorized queries", async (t) => {
  ...
  for (const [type, expected] of [
    ["ui_component", "progress-track"],
    ["business_object", "reward-stage"],
    ["state", "claimed"],
    ["layout_scene", "RTL"],
    ["issue_symptom", "progress-display-mismatch"],
  ])
    assert.ok(
      values(payload, type).includes(expected),
      `${type}: ${JSON.stringify(payload.entities[type])}`,
    );
  assertCategorizedQueries(payload);
  for (const suffix of ["公司 Docs", "Skill", "当前项目已有实现"]) {
    assert.ok(
      !payload.retrieval_queries.includes(`${prompt} ${suffix}`),
      JSON.stringify(payload.retrieval_queries),
    );
  }
  for (const text of [
    "研发概念：",
...
test("live visual and interaction inspection uses inspection intent and skill query", async (t) => {
  ...
  assert.deepEqual(payload.retrieval_query_groups.skills, [
    "浏览器即时视觉与交互检查",
  ]);
  assert.ok(
    payload.retrieval_query_groups.skills.every(
      (query) => !query.includes("Bug") && !query.includes("修复"),
    ),
  );
  assertCategorizedQueries(payload);
});
...
test("exact code symbols are queried only when they appear in real input", async (t) => {
  ...
  assert.ok(
    !values(absent, "config_or_symbol").includes("progressRewardConfig"),
  );
  assert.ok(
    !absent.retrieval_queries.some((query) =>
      query.includes("progressRewardConfig"),
    ),
  );
  assert.ok(
    values(present, "config_or_symbol").includes("progressRewardConfig"),
  );
  assert.ok(
    present.retrieval_query_groups.code.includes("progressRewardConfig"),
  );
...
test("reward mask request extracts UI semantics without treating the asset as a directory", async (t) => {
  ...
  for (const [type, expected] of [
    ["task", "ui-modification"],
    ["business_object", "reward-item"],
    ["state", "claimed"],
    ["visual_effect", "mask"],
    ["asset_resource", "icon/mask"],
  ])
    assert.ok(
      values(payload, type).includes(expected),
      `${type}: ${JSON.stringify(payload.entities[type])}`,
  ...
  assert.ok(
    !payload.confirmed_context.some((item) =>
      ["target_file", "target_directory"].includes(item.type),
    ),
  );
...
test("technical identifiers are categorized by semantics instead of punctuation", async (t) => {
  ...
  assert.deepEqual(
    payload.confirmed_context.map((item) => [item.type, item.value]),
    [
  ...
  assert.ok(
    values(payload, "config_or_symbol").includes("progressRewardConfig"),
  );
...
test("skill choice is explained only for a real overlapping deliverable", async (t) => {
  ...
  assert.ok(
    ambiguous.includes("选型说明：任务同时提到方案与代码实施"),
    ambiguous,
  );
...
test("legacy profile protocol stays disabled", async () => {
  await assert.rejects(
    run(
      process.execPath,
      [SCRIPT, "--query", "开发弹窗", "--profile-json", "{}"],
      { encoding: "utf8" },
    ),
    (error) =>
      error.code === 2 &&
      error.stderr.includes("Unknown argument: --profile-json"),
  );
