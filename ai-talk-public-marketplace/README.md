...
AI Talk 让 AI 开发任务连续、有边界且可验证：**连续保存任务状态，按风险渐进升级，并把实际形成的决策结论说明白。** 安装后直接描述研发需求，不需要命令前缀；状态询问、纯引用和非研发消息仍原样放行。

```text
研发请求
→ 延续：复用目标、授权、范围和已有证据，不逐轮重问
→ 升级：简单任务保持轻量，真实风险出现前才启用完整契约
→ 可见：实际介入时报告判断、依据和影响，不暴露内部思维链
→ 完成：报告实际结果；有介入时说明 AI Talk 的已证实贡献
```

明确、可逆的局部修改静默走无契约 Fast Path。复杂任务内部使用 `RequirementContract 1.4` 锁定目标、边界和验收，但不展示协议字段、YAML 或思维链。AI Talk 实际完成目标绑定、边界决策、诊断证据链、风险升级或有影响的状态复用时，会用最多三行说明判断、依据和影响；只有答案会改变产品结果、数据语义或允许写入范围时才询问一个问题。

Fast Path 调查后若发现跨模块行为、数据语义、范围限制、专用流程或外部写入等真实风险，会在首次扩大边界的写入前动态升级并保留已有证据，同时说明升级依据和实际影响。只多涉及一个相邻实现、类型或测试文件不会触发升级；确需扩大权限或决定产品结果时才询问。

“为什么”“只分析”“不要修改”等明确诊断请求保持只读。在代码工作区中，目标与预期明确、局部可逆且不涉及外部写入的纯缺陷陈述会直接修复并验证；目标或预期不明、仅提供证据以及生产或外部系统状态缺陷仍保持只读。只读诊断会维护 `DiagnosticBrief`，把目标、带来源的事实、可证伪假设、验证链和停止条件交给后续排查；随后说“执行”时复用证据和已完成检查，不重新扫描。外部写入必须由直接命令授权，或由简短肯定紧邻确认上一轮已列清系统、单一动作和全部目标的提议；同一任务的精确授权可复用，但不得扩展到新目标或新动作。

视觉请求先检查已有的新鲜截图标注、DOM 选择、IDE 选区、文件行号或业务 ID；目标唯一时直接继续，多义或陈旧时才请求一次补充。`excluded_scope` 和 `bounded` 范围通过前后工作区快照强制校验，越界时不得声称完成。

完成时沿用普通 Agent 的终态表达，报告实际改动、静态检查、运行验证和未验证项。发生实际介入时增加一句 `AI Talk 贡献`，只描述当前对话或活动状态能证明的影响；纯 Fast Path `skip` 不显示也不冒领贡献。没有 scope guard 证据时只列实际改动范围，不虚构范围保护结论。

Fast Path 每轮只读取主 `SKILL.md` 1 次，仍保持零协议 reference、AI Talk 仓库、contract checker 和 reporter 调用。纯 `skip` 不显示提示；`light_binding`、`DiagnosticBrief` 或有实质影响的状态复用可以显示一次决策摘要。契约校验器继续确定性检查结构、真实文件与行号、视觉证据、范围冲突和外部写入授权。
...
Strict Mode 只提高触发覆盖率，不改变决策可见性和风险边界行为。非研发对话不使用 AI Talk；`$ai-talk:...` 仅保留为兼容的显式调用方式。
